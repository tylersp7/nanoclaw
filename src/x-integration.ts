 ...
 *
 * Handles x_* IPC messages from container agents by spawning
 * Playwright scripts that automate Chrome for X interactions.
 *
 * Based on .claude/skills/x-integration/host.ts, adapted for
 * project compilation (src/ rootDir) and project logger.
 */
...
export async function handleXIpc(
  ...
  writeResult(sourceGroup, requestId, result);
  if (result.success) {
